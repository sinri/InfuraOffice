<template>
    <div>
        ...
        <Row>
            <i-col span="24"><p>Call preset shell functions to do mixed jobs, including explosion, antiquity and zombie
                removal.</p></i-col>
        </Row>
        <Row>
            <i-col span="16">
        ...
        <Row v-for="(item,key) in draft.explosion_list" class="explosion_list_config_div">
            <i-col span="10">
                <i-input v-model="item.keep_tail_lines"><span slot="prepend">Keep Tail Lines:</span></i-input>
            </i-col>
            <i-col span="10">
            ...
            <i-col span="4">
                <i-button v-on:click="remove_explosion_list_item(key)" icon="android-cancel" type="error">Delete
                </i-button>
            </i-col>
            <i-col span="24">
        ...
        <Row v-for="(item,key) in draft.antiquity_list" class="antiquity_list_config_div">
            <i-col span="10">
                <i-input v-model="item.not_modified_days"><span slot="prepend">Not Modified Days:</span></i-input>
            </i-col>
            <i-col span="10"></i-col>
            <i-col span="4">
                <i-button v-on:click="remove_antiquity_list_item(key)" icon="android-cancel" type="error">Delete
                </i-button>
            </i-col>
            <i-col span="24">
                <i-input type="textarea" v-model="item.joined_files" placeholder="file patterns in lines">
        ...
        <Row v-for="(item,key) in draft.zombie_list" class="zombie_list_config_div">
            <i-col span="10">
                <i-input v-model="item.not_accessed_days"><span slot="prepend">Not Accessed Days:</span></i-input>
            </i-col>
            <i-col span="10"></i-col>
            <i-col span="4">
                <i-button v-on:click="remove_zombie_list_item(key)" icon="android-cancel" type="error">Delete</i-button>
            </i-col>
            <i-col span="24">
                <i-input type="textarea" v-model="item.joined_files" placeholder="file patterns in lines">
                    <span slot="prepend">Files:</span>
        ...
        methods: {
            ...
            load_existed_mixed_jobs: function () {
                ...
                Tools.callInfuraOfficeJsonAPI(
                    "post",
                    "/api/JobConfigController/jobs/MixedJob",
                    {},
                    (data) => {
                        this.$Loading.finish();
...
<style scoped>
    ...
    .explosion_list_config_div {
        margin: 10px;
        border: 1px solid lightgrey;
    }

    .explosion_list_config_div > div {
        padding: 10px;
    }

    .antiquity_list_config_div {
        margin: 10px;
        border: 1px solid lightgrey;
    }

    .antiquity_list_config_div > div {
        padding: 10px;
    }

    .zombie_list_config_div {
        margin: 10px;
        border: 1px solid lightgrey;
    }

    .zombie_list_config_div > div {
        padding: 10px;
    }
</style>